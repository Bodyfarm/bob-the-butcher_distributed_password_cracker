...
static void blowfish_init(unsigned char * key, short keybytes)
{
  int i, j;
  uint32_t data;
  uint32_t datal;
  ...
  /* uh... this is over 4k */
  box.P = (uint32_t *) malloc((bf_N + 2) * sizeof(uint32_t));
  box.S = (uint32_t **) malloc(4 * sizeof(uint32_t *));
  for (i = 0; i < 4; i++)
    box.S[i] = (uint32_t *) malloc(256 * sizeof(uint32_t));
  bf_P = box.P;
  bf_S = box.S;
